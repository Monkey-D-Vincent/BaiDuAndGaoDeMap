...
import com.xianzhi.map.gaode.bean.BarChartData;
import com.xianzhi.map.gaode.util.util.DisplayUtil;
import com.xianzhi.map.gaode.util.util.StringUtil;

import java.util.List;

/**
 * @author liMing
 * @Demo class BarChartView
 * @Description TODO
 * @date 2019-11-07 15:30
 */
public class BarChartView extends View {
	...
	private int getMaxScrollX(int direction) {
		if (direction > 0) {
			return barChartTotalWidth - getScrollX() - width + paddingStart + paddingEnd > 0 ? barChartTotalWidth - getScrollX() - width + paddingStart + paddingEnd : 0;
		} else if (direction < 0) {
			return getScrollX();
		}
		return 0;
		...
		if (dataList != null && dataList.size() > 0) {
			...
			for (BarChartData date : dataList) {
				...
				if (barChartShaderColor != null && barChartShaderColor.length > 0) {
					shader = getbarChartShader(barChartPaintRect.left, chartPaddingTop + valueTobarChartDistance + valueTextSize, barChartPaintRect.right, barChartPaintRect.bottom, barChartShaderColor);
				}
